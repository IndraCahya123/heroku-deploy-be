...
exports.getAllProducts = async (req, res) => {
    try {
        ...
        const productsObject = JSON.parse(productString);

        const productsModifiedImg = productsObject.map(product => {
            return {
                ...product,
                image: process.env.IMG_URL + product.image,
                user: {
                    ...product.user,
                    image: process.env.IMG_URL + product.user.image,
                }
            }
        })
...
exports.getAllProductsByPartnerId = async (req, res) => {
    try {
        ...
        const productsModified = productsObject.map(product => {
            return {
                ...product,
                image: process.env.IMG_URL + product.image,
                user: {
...
exports.addNewProduct = async (req, res) => {
    try {
        ...
        const user = await User.findOne({
            ...
            attributes: {
                exclude: ["password", "image", "email", "location", "phone", "createdAt", "updatedAt"]
            }
        });
